...
void CheckHeartBeat(UINT64 numIter, UINT64 numMispred, FILE* fp) {
    UINT64 d1K = 1000;
    UINT64 d10K = 10000;
    UINT64 d100K = 100000;
    UINT64 d1M = 1000000;
...
int main(int argc, char *argv[]) {
    ...
    PyObject *brpredRecordResult = PyObject_GetAttrString(brpred, "RecordResult");
    if (brpredRecordResult == NULL || !PyCallable_Check(brpredRecordResult)) {
        if (PyErr_Occurred())
            PyErr_Print();
        fprintf(stderr, "Fatal error: cannot use the PREDICTOR RecordResult method\n");
        Py_XDECREF(brpredRecordResult);
        Py_DECREF(brpred);
        pythonCleanup(program);
        return 1;
    }
...
//    UINT64 current_instruction_counter = 0;
    ...
    for (auto it = bt9_reader.begin(); it != bt9_reader.end(); ++it) {
        CheckHeartBeat(++numIter, numMispred, fp); //Here numIter will be equal to number of branches read
...
/************************************************************************************************************/
            ...
            if (opType == OPTYPE_ERROR) {
                ...
                // predDir = brpred->GetPrediction(PC, Speculative_PC);
                PyTempValue = Py_BuildValue("(I,I)", PC, branchTarget);
...
//                brpred->UpdatePredictor(PC, opType, branchTaken, predDir, branchTarget);
                PyTempValue = Py_BuildValue("(I,l,i,i,I,I,I)", PC, opType, branchTaken, predDir, branchTarget, numIter, branch_instruction_counter);
...
//              brpred->RecordResult(numIter, numMispred, branch_instruction_counter);
            PyTempValue = Py_BuildValue("(I,I,l,i,I,I,I)", PC, previous_PC, opType, branchTaken, branchTarget, numIter, branch_instruction_counter);
            ...
            PyTempValue = Py_BuildValue("(I,I,I)", numIter, numMispred, branch_instruction_counter);
            PyTemp = PyObject_CallObject(brpredRecordResult, PyTempValue);
            if (PyTemp == NULL) {
                PyErr_Print();
                fprintf(stderr, "Fatal error: did not call PREDICTOR RecordResult successfully.");
                Py_XDECREF(PyTempValue);
                Py_DECREF(brpredGetPrediction);
                Py_DECREF(brpredUpdatePredictor);
                Py_DECREF(brpredTrackOtherInst);
...
/************************************************************************************************************/
    ...
    fprintf(fp,"%s%s%s", "TRACE \t : ", trace_path.c_str(), "\n") ;
    printf("  NUM_INSTRUCTIONS            \t : %10llu", total_instruction_counter);
    fprintf(fp,"%s%10llu%s", "NUM_INSTRUCTIONS            \t : ", total_instruction_counter, "\n") ;
    printf("  NUM_BR                      \t : %10llu",
           branch_instruction_counter - 1); //JD2_2_2016 NOTE there is a dummy branch at the beginning of the trace...
    fprintf(fp,"%s%10llu%s", "NUM_BR                      \t : ", branch_instruction_counter - 1, "\n") ;
    printf("  NUM_UNCOND_BR               \t : %10llu", uncond_branch_instruction_counter);
    fprintf(fp,"%s%10llu%s", "NUM_UNCOND_BR               \t : ", uncond_branch_instruction_counter, "\n") ;
    printf("  NUM_CONDITIONAL_BR          \t : %10llu", cond_branch_instruction_counter);
    fprintf(fp,"%s%10llu%s", "NUM_CONDITIONAL_BR               \t : ", cond_branch_instruction_counter, "\n") ;
    printf("  NUM_MISPREDICTIONS          \t : %10llu", numMispred);
    fprintf(fp,"%s%10llu%s", "NUM_MISPREDICTIONS               \t : ", numMispred, "\n") ;
    printf("  MISPRED_PER_1K_INST         \t : %10.4f",
           1000.0 * (double) (numMispred) / (double) (total_instruction_counter));
    fprintf(fp,"%s%10.4f%s", "MISPRED_PER_1K_INST               \t : ", 1000.0 * (double) (numMispred) / (double) (total_instruction_counter), "\n") ;
    fclose(fp) ;
    printf("\n");
