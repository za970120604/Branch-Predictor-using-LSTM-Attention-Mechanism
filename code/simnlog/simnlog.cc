...
int main(int argc, char *argv[]) {
    ...
    FILE *fp_rollingAcc ; 
    char buffer[500];
    sprintf(buffer, "./log/%s.txt", argv[2]);
    printf("%s\n", buffer);
    fp_log = fopen(buffer, "w+");
    sprintf(buffer, "./tage_sc_l/numMispred/%s_numMispred.txt", argv[2]);
    printf("%s\n", buffer);
    fp_numMispred = fopen(buffer, "w+");
    sprintf(buffer, "./tage_sc_l/rollingAcc/%s_rollingAcc.txt", argv[2]);
    printf("%s\n", buffer);
    fp_rollingAcc = fopen(buffer, "w+");
    ...
    for (auto it = bt9_reader.begin(); it != bt9_reader.end(); ++it) {
        CheckHeartBeat(++numIter, numMispred, fp_log); //Here numIter will be equal to number of branches read
...
/************************************************************************************************************/
            ...
            fprintf(fp_numMispred, "%llu\n", numMispred); 
            fprintf(fp_rollingAcc, "%.4f\n", double(numMispred)/double(numIter)); 
...
/************************************************************************************************************/
    ...
    fprintf(fp_log,"%s%s%s", "TRACE \t : ", trace_path.c_str(), "\n") ;
    printf("  NUM_INSTRUCTIONS            \t : %10llu", total_instruction_counter);
    fprintf(fp_log,"%s%10llu%s", "NUM_INSTRUCTIONS            \t : ", total_instruction_counter, "\n") ;
    printf("  NUM_BR                      \t : %10llu",
           branch_instruction_counter - 1); //JD2_2_2016 NOTE there is a dummy branch at the beginning of the trace...
    fprintf(fp_log,"%s%10llu%s", "NUM_BR                      \t : ", branch_instruction_counter - 1, "\n") ;
    printf("  NUM_UNCOND_BR               \t : %10llu", uncond_branch_instruction_counter);
    fprintf(fp_log,"%s%10llu%s", "NUM_UNCOND_BR               \t : ", uncond_branch_instruction_counter, "\n") ;
    printf("  NUM_CONDITIONAL_BR          \t : %10llu", cond_branch_instruction_counter);
    fprintf(fp_log,"%s%10llu%s", "NUM_CONDITIONAL_BR               \t : ", cond_branch_instruction_counter, "\n") ;
    printf("  NUM_MISPREDICTIONS          \t : %10llu", numMispred);
    fprintf(fp_log,"%s%10llu%s", "NUM_MISPREDICTIONS               \t : ", numMispred, "\n") ;
    printf("  MISPRED_PER_1K_INST         \t : %10.4f",
           1000.0 * (double) (numMispred) / (double) (total_instruction_counter));
    fprintf(fp_log,"%s%10.4f%s", "MISPRED_PER_1K_INST               \t : ", 1000.0 * (double) (numMispred) / (double) (total_instruction_counter), "\n") ;
    printf("\n");
    fclose(fp_log) ; 
    fclose(fp_numMispred) ;
    fclose(fp_rollingAcc) ;
}
